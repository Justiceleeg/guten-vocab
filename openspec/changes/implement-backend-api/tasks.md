...
- [ ] 1.1 Create `backend/app/schemas/` directory and `__init__.py`
- [ ] 1.2 Create `backend/app/api/routes/` directory structure
- [ ] 1.3 Create `backend/app/api/routes/__init__.py`
- [ ] 1.4 Create `backend/app/api/routes/students.py` for student endpoints
- [ ] 1.5 Create `backend/app/api/routes/class.py` for class endpoints
- [ ] 1.6 Create `backend/app/api/routes/books.py` for books endpoint (optional)

## 2. Create Pydantic Schemas
- [ ] 2.1 Create `backend/app/schemas/student.py` with:
...
- [ ] 2.2 Create `backend/app/schemas/class.py` with:
...
- [ ] 2.3 Create `backend/app/schemas/book.py` with:
  - `BookListResponse` schema (id, title, author, reading_level, total_words)

## 3. Create Service Layer
- [ ] 3.1 Create `backend/app/services/student_service.py` with:
...
- [ ] 3.2 Create `backend/app/services/class_service.py` with:
...
- [ ] 3.3 Create `backend/app/services/recommendation_service.py` with:
  - `get_student_recommendations(db, student_id)` - Query student recommendations with book details
- [ ] 3.4 Create `backend/app/services/__init__.py` to export services

## 4. Implement Student Endpoints
- [ ] 4.1 Implement `GET /api/students` in `backend/app/api/routes/students.py`:
  - Use `get_all_students` service
  - Return list of `StudentListResponse` schemas
  - Handle database errors
- [ ] 4.2 Implement `GET /api/students/{id}` in `backend/app/api/routes/students.py`:
...
- [ ] 5.1 Implement `GET /api/class/stats` in `backend/app/api/routes/class.py`:
  - Use `get_class_stats` service
  - Return `ClassStatsResponse` schema
  - Handle database errors
- [ ] 5.2 Implement `GET /api/class/recommendations` in `backend/app/api/routes/class.py`:
...
- [ ] 6.1 Implement `GET /api/books` in `backend/app/api/routes/books.py`:
...
- [ ] 7.1 Import route routers in `backend/app/main.py`
- [ ] 7.2 Register student routes with `app.include_router(students_router, prefix="/api/students", tags=["students"])`
- [ ] 7.3 Register class routes with `app.include_router(class_router, prefix="/api/class", tags=["class"])`
- [ ] 7.4 Register books routes with `app.include_router(books_router, prefix="/api/books", tags=["books"])` (if implemented)

## 8. Error Handling and Validation
- [ ] 8.1 Add 404 error handler for missing students in student endpoints
- [ ] 8.2 Add 500 error handler for database/server errors
- [ ] 8.3 Validate student_id is integer in path parameters
- [ ] 8.4 Verify CORS configuration in `main.py` allows frontend origin
